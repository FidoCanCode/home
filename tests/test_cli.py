...
from kennel.cli import build_parser, cmd_add, cmd_complete, cmd_list, main
...
class TestCmdAdd:
    def test_adds_task(self, tmp_path: Path) -> None:
        _task_file(tmp_path)
        cmd_add(tmp_path, "my task", "some description")
...
class TestCmdComplete:
    def test_completes_task_no_thread(self, tmp_path: Path) -> None:
        _task_file(tmp_path)
        cmd_add(tmp_path, "task to finish", "")
        cmd_complete(tmp_path, "task to finish")
        from kennel.tasks import list_tasks
    ...
    def test_completes_task_with_thread_resolves(self, tmp_path: Path) -> None:
        ...
        with (
            patch("kennel.cli.github.get_user", return_value="fido-bot"),
            patch(
                "kennel.cli.github.get_pull_comments",
                return_value=[
                    {
                        "id": 42,
                        "in_reply_to_id": None,
            ...
            patch(
                "kennel.cli.github.get_review_threads",
                return_value={
                    "data": {
                        "repository": {
                            "pullRequest": {
            ...
            patch("kennel.cli.github.resolve_thread") as mock_resolve,
        ):
            cmd_complete(tmp_path, "threaded task")

        mock_resolve.assert_called_once_with("thread_node_abc")
    ...
    def test_completes_task_with_thread_no_matching_comments(
        ...
        with (
            patch("kennel.cli.github.get_user", return_value="fido-bot"),
            patch("kennel.cli.github.get_pull_comments", return_value=[]),
            patch("kennel.cli.github.resolve_thread") as mock_resolve,
        ):
            cmd_complete(tmp_path, "threaded task")
    ...
    def test_completes_task_with_thread_exception_silenced(
        ...
        with patch(
            "kennel.cli.github.get_user", side_effect=RuntimeError("network error")
        ):
    ...
    def test_thread_missing_fields_skips(self, tmp_path: Path) -> None:
        ...
        with patch("kennel.cli.github.resolve_thread") as mock_resolve:
            cmd_complete(tmp_path, "task")
...
class TestCmdList:
    def test_prints_json(self, tmp_path: Path, capsys) -> None:
        _task_file(tmp_path)
        cmd_add(tmp_path, "alpha", "")
        cmd_add(tmp_path, "beta", "desc")
        cmd_list(tmp_path)
