...
class GitHub:
    ...
    def find_issues(self, owner: str, repo: str, login: str) -> list[dict[str, Any]]:
        ...
        query = (
            ...
            f"subIssues(first:50,orderBy:{{field:PRIORITY,direction:ASC}}){{nodes{{state {issue_fields}}} "
    ...
    def find_all_open_issues(self, owner: str, repo: str) -> list[dict[str, Any]]:
        ...
        query = (
            ...
            "orderBy:{field:CREATED_AT,direction:ASC}){"
            f"nodes{{{issue_fields} state "
            f"subIssues(first:100,orderBy:{{field:PRIORITY,direction:ASC}}){{nodes{{state {issue_fields}}}}}"
            "pageInfo{endCursor hasNextPage}"
            "}}}"
        )
        issues: list[dict[str, Any]] = []
    ...
    def get_sub_issues(
        ...
        query = (
            ...
            "repository(owner:$owner,name:$repo){"
            "issue(number:$number){"
            "subIssues(first:50,after:$cursor,orderBy:{field:PRIORITY,direction:ASC}){"
