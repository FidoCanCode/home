...
let preamble _state _name _comment _used_modules _safe =
  ...
  str "class _Impossible(Exception): pass  # catch-all for non-exhaustive matches" ++ fnl ()
  ...
  | MLcase (_, scrutinee, branches) ->
      ...
      if is_bool then
        ...
        (* Append a catch-all arm unless the last explicit branch is already
           ...
           when no arm matches; [_Impossible] makes that a loud runtime error
           so non-exhaustive patterns are never silent. *)
        let catch_all =
  ...
  | MLcase (_, scrutinee, branches) as expr ->
      ...
      if is_bool then
        ...
        (* Same catch-all logic as the expression-context path: append
           [case _: raise _Impossible()] unless the last explicit arm is
           already a wildcard. *)
        let catch_all =
          if has_wildcard_last branches then mt ()
          else fnl () ++ str case_pfx ++ str "case _:" ++ fnl () ++
               str body_pfx ++ str "raise _Impossible()"
        in
        str "match " ++ pp_expr state env scrutinee ++ str ":" ++ fnl () ++
        prlist_with_sep fnl pp_branch (Array.to_list branches) ++
        catch_all
