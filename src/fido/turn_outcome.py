...
from fido.rocq.turn_outcome import (
    parse_sentinel as _rocq_parse_sentinel,
)
...
class TurnOutcomeParser:
    """Parses turn_outcome sentinels with an injectable Rocq oracle for validation.

    The *sentinel_oracle* callable wraps the Rocq-proven
    :func:`~fido.rocq.turn_outcome.parse_sentinel` so tests can inject a
    fake oracle that diverges from the parser — exercising the
    AssertionError paths — without patching the Rocq-extracted module.

    Production code uses the module-level :func:`parse_turn_outcome` convenience
    wrapper, which delegates to a singleton wired to the real oracle.
    """

    def __init__(
        self,
        sentinel_oracle: Callable[[str, str], TurnOutcome | None],
    ) -> None:
        self._sentinel_oracle = sentinel_oracle

    def _assert_parse_oracle(
        self, kind: str, payload: str, result: TurnOutcome
    ) -> None:
        """Assert that the injected oracle agrees with our dispatch."""
        oracle = self._sentinel_oracle(kind, payload)
        if oracle is None:
    ...
    def _assert_reject_oracle(self, kind: str, payload: str) -> None:
        """Assert that the injected oracle also rejects this input.
    ...
    def _require_nonempty_str(
        self, obj: dict[str, object], field: str, kind: str
    ) -> str:
        ...
        if not isinstance(value, str) or not value.strip():
            # When the value IS a string (but empty/whitespace), assert the
            # oracle also rejects.  Python normalizes by stripping — the
            # model rejects empty payloads — so we pass the stripped form.
            if isinstance(value, str):
                self._assert_reject_oracle(kind, value.strip())
            raise ValueError(
    ...
    def _parse_outcome_field(
        ...
        """Validate *field* in *obj*, construct the outcome via *make*, assert oracle."""
        payload = self._require_nonempty_str(obj, field, kind)
        result = make(payload)
        self._assert_parse_oracle(kind, payload, result)
        return result

    def parse(self, text: str) -> "TurnOutcomeBundle":
        ...
        try:
            ...
            raise ValueError(
                f"Last non-empty line is not valid JSON: {last!r}"
            ) from None
        ...
        match kind:
            ...
            case _:
                ...
                self._assert_reject_oracle(kind, "x")
...
_parser = TurnOutcomeParser(sentinel_oracle=_rocq_parse_sentinel)


def parse_turn_outcome(text: str) -> TurnOutcomeBundle:
    """Parse the turn_outcome sentinel from the last non-empty line of *text*.

    Thin wrapper around the module-level :class:`TurnOutcomeParser` singleton
    wired to the real Rocq oracle.  Callers that need to inject a different
    oracle (e.g. tests verifying divergence detection) should construct a
    :class:`TurnOutcomeParser` directly.
    """
