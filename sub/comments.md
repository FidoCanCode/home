...
```bash
uv run --project /home/rhencke/workspace/fido fido task <work_dir> list | jq '[.[] | select(.status == "pending" and (.title | startswith("ASK:")))]'
```

For each ASK task with a thread, check if the human has replied:
- **Still unclear** → post another follow-up, leave the task open.
- **Clear enough to act** → create a new task via fido task, mark the ASK task complete:
  ```bash
  uv run --project /home/rhencke/workspace/fido fido task <work_dir> add "<new task title>"
  uv run --project /home/rhencke/workspace/fido fido task <work_dir> complete "ASK: <original title>"
  ```
...
- **DO** — worth implementing:
  ```bash
  uv run --project /home/rhencke/workspace/fido fido task <work_dir> add "PR comment: <short summary>" --comment-id <first_db_id> --repo <repo> --pr <pr>
  ```
...
- **ASK** — unclear what to do:
  ...
  ```bash
  uv run --project /home/rhencke/workspace/fido fido task <work_dir> add "ASK: <short summary>" --comment-id <first_db_id> --repo <repo> --pr <pr>
  ```
...
Do NOT edit the PR body directly. The fido server syncs it automatically after task changes.
