...
```bash
cd /home/rhencke/home-runner && ./fido task <work_dir> list | jq '[.[] | select(.status == "pending" and (.title | startswith("ASK:")))]'
```
...
- **Clear enough to act** → create a new task via `./fido task`, mark the ASK task complete:
  ```bash
  cd /home/rhencke/home-runner && ./fido task <work_dir> add "<new task title>"
  cd /home/rhencke/home-runner && ./fido task <work_dir> complete "ASK: <original title>"
  ```
...
- **DO** — worth implementing:
  ```bash
  cd /home/rhencke/home-runner && ./fido task <work_dir> add "PR comment: <short summary>" --comment-id <first_db_id> --repo <repo> --pr <pr>
  ```
...
- **ASK** — unclear what to do:
  ...
  ```bash
  cd /home/rhencke/home-runner && ./fido task <work_dir> add "ASK: <short summary>" --comment-id <first_db_id> --repo <repo> --pr <pr>
  ```
