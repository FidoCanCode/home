...
```bash
bash /home/rhencke/workspace/kennel/task-cli.sh <work_dir> list | jq '[.[] | select(.status == "pending" and (.title | startswith("ASK:")))]'
```

For each ASK task with a thread, check if the human has replied:
- **Still unclear** → post another follow-up, leave the task open.
- **Clear enough to act** → create a new task via task-cli.sh, mark the ASK task complete:
  ```bash
  bash /home/rhencke/workspace/kennel/task-cli.sh <work_dir> add "<new task title>"
  bash /home/rhencke/workspace/kennel/task-cli.sh <work_dir> complete "ASK: <original title>"
  ```
...
- **DO** — worth implementing:
  ```bash
  bash /home/rhencke/workspace/kennel/task-cli.sh <work_dir> add "PR comment: <short summary>"
  ```
...
- **ASK** — unclear what to do:
  ...
  ```bash
  bash /home/rhencke/workspace/kennel/task-cli.sh <work_dir> add "ASK: <short summary>"
  ```
